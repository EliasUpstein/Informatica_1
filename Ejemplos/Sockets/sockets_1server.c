...
int main(int argc, char *argv[])
{
	...
	serv_addr.sin_addr.s_addr = INADDR_ANY;		// Vamos a aceptar cualquier direcciÃ³n de cliente
	...
	// Paso4 accept() que se queda (bloqueado) esperando una conexiÃ³n del cliente
	newsockfd = accept(sockfd, (struct sockaddr *) &cli_addr, &clilen);
		...
		// De estos sockets tendremos uno por cliente (cuando podamos aceptar mÃ¡s de un cliente en programas futuros)
	printf("\tConexion aceptada!\n");

	// Paso5 -> Ya nos podemos comunicar a travÃ©s de newsockfd
	if (newsockfd < 0)
