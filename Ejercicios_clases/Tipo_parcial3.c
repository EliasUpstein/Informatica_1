...
int main(int cantArg, char *args[])
{
    ...
    if (cantArg > 1)
    {
        ...
        // printf("Ingrese 10 palabras prohibidas:\n");
        // fflush(stdin);
        // scanf("%s %s %s %s %s %s %s %s %s %s", )  tendría que declarar 10 strings?

        for (int i = 1; i < cantArg; i++)           //Para cada string pasado como argumento
        {
            for (int j = 0; i < 10; j++)            //Para cada palabra ingresada como prohibida
            {
                if (strcmp(args[i], direcciones[j]) == 0)       //Si los string son iguales
                {
                    for (int k = 0; k < strlen(args[i]); i++)   //Para cada letra del sting escribe X
                    {
                            args[i][k] = 'X';
                            count++;
                    }
